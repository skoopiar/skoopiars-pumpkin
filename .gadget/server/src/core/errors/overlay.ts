...
export function patchOverlay(code: string, environment: string): string {
  let codeToReplace, replacementCode;
  if (code.includes("class ErrorOverlay")) {
    codeToReplace = "class ErrorOverlay";
    replacementCode = getOverlayCode(environment) + "\nclass ViteErrorOverlay";
  } else if (code.includes("var ErrorOverlay")) {
    codeToReplace = "var ErrorOverlay =";
    replacementCode = getOverlayCode(environment) + "\nvar ViteErrorOverlay =";
  } else {
    return code;
  }
  let result = code.replace(codeToReplace, replacementCode);
