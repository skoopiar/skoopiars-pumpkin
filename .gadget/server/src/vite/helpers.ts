...
export const getFrontendTypeFromIndicatorFile = async (): Promise<FrontendType> => {
  let type = FrontendType.Vite;

  try {
    const frontendIndicator = (await fs.readFile(frontendTypeIndicatorFilePath, "utf-8")).trim() as FrontendType;
    if (frontendIndicator === FrontendType.Remix) {
      type = FrontendType.Remix;
    } else if (frontendIndicator === FrontendType.ReactRouterFramework) {
      type = FrontendType.ReactRouterFramework;
    } else if (frontendIndicator === FrontendType.Vite) {
      type = FrontendType.Vite;
    }
  } catch (error) {
