 ...
 */
export const internalModelManagerForModel = (api: AnyClient, apiIdentifier: string, namespace: string[]): AnyInternalModelManager => {
  const modelPath = [...namespace, apiIdentifier];
  const manager: AnyInternalModelManager | undefined = Globals.platformModules.lodash().get(api, ["internal", ...modelPath]);
  if (!manager) {
 ...
 * Get the internal model manager for the model from its maybe-namespaced spot
 */
export const internalModelManagerForTypename = (api: AnyClient, typename: string): AnyInternalModelManager => {
...
export async function legacySuccessfulAuthentication(params: AnyParams): Promise<void> {
  ...
  const manager = api.internal.user as AnyInternalModelManager;
