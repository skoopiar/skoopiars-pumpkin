import { NoTransitionError } from "../errors.js";
 ...
 */ export const possibleTransitionStates = ({ record, model }, mapper)=>{
 ...
 */ export const checkCanExecute = (context, mapper)=>{
    const receivingState = Globals.platformModules.lodash().find(possibleTransitionStates(context, mapper), [
        "blob.key",
    ...
    if (!receivingState) {
        ...
        if (Globals.platformModules.lodash().isString(state)) {
            ...
            const state = Globals.platformModules.lodash().find(context.model.stateChart.childStates, {
                key: initialStateKey
            });
            if (state) {
 ...
 */ export const computeStateHistory = (history, state)=>{
    const newHistory = history ? Globals.platformModules.klona().klona(history) : {
        current: ""
    };
    const path = [];
 ...
 */ export const computeStateValueFor = (history, state, mapper)=>{
    ...
    while(state.blob.initialChildStateKey){
        ...
        if (state.blob.restoreHistory) {
            ...
            const maybeChildState = Globals.platformModules.lodash().find(state.blob.childStates, {
                apiIdentifier
            });
            if (maybeChildState) {
...
export const _doStateTransition = (currentHistory, currentState, toState, mapper, record)=>{
...
export const doStateTransition = (context, mapper)=>{
...
export const persistStateTransition = async (newState, newHistory, context)=>{
    if (context.record.changed("state") || context.record.changed("stateHistory")) {
        await updateState(context, newState, newHistory);
    }
};
/** Persist the new state into the model record */ export const updateState = async ({ api, model, record, scope, logger }, newState, stateHistory)=>{
